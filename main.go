...
func detectColumnMapping(rows [][]string) ColumnMapping {
	...
	dateKeywords := []string{"تاریخ", "date", "tarikh", "شرح"}
	withdrawKeywords := []string{"برداشت", "withdraw", "debit", "بدهکار"}
	depositKeywords := []string{"واریز", "deposit", "credit", "بستانکار"}
	...
	for rowIdx, row := range rows {
		...
		for colIdx, cell := range row {
			cellLower := strings.TrimSpace(strings.ToLower(cell))

			if containsKeyword(cellLower, dateKeywords) {
				mapping.DateCol = colIdx
				isHeader = true
			}
			if containsKeyword(cellLower, withdrawKeywords) {
				mapping.WithdrawCol = colIdx
				isHeader = true
			}
			if containsKeyword(cellLower, depositKeywords) {
				mapping.DepositCol = colIdx
				isHeader = true
			}
		}
		...
		// If header already found, stop looking
		if headerFound {
			break
		}
		...
		for colIdx, cell := range row {
			...
			if strings.Contains(cell, "/") && len(cell) >= 8 && len(cell) <= 10 {
				// Make sure it's not just a number with slash
...
func excelToJSON(filePath string) (*FileData, error) {
	...
	for i, row := range rows {
		...
		// Skip if date is actually a row number
