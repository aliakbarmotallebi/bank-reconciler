...
func detectColumnMapping(headers []string) ColumnMapping {
	...
	for i, h := range headers {
		...
		if strings.Contains(h, "date") || strings.Contains(h, "tarikh") || strings.Contains(h, "tarix") {
			mapping.DateCol = i
			continue
		}

		if strings.Contains(h, "variz") || strings.Contains(h, "deposit") || strings.Contains(h, "credit") ||
			strings.Contains(h, "bestankar") || strings.Contains(h, "bedehkar") {
			mapping.DepositCol = i
			continue
		}

		if strings.Contains(h, "bardasht") || strings.Contains(h, "withdraw") || strings.Contains(h, "debit") ||
			strings.Contains(h, "bedehkar_") {
			mapping.WithdrawCol = i
			continue
		}
	}
...
func autoDetectMapping(rows [][]string) ColumnMapping {
	mapping := ColumnMapping{
		DateCol:     -1,
		DepositCol:  -1,
		WithdrawCol: -1,
	...
	for rowIdx, row := range rows {
		...
		for colIdx, cell := range row {
			...
			if strings.Contains(cell, "/") && len(cell) >= 8 {
			...
			if _, err := strconv.ParseFloat(strings.ReplaceAll(cell, ",", ""), 64); err == nil {
				hasNumber = true
				if mapping.WithdrawCol == -1 && colIdx != mapping.DateCol {
					mapping.WithdrawCol = colIdx
				}
			}
		}
		...
		detected := detectColumnMapping(row)
		if detected.DateCol != -1 {
			mapping = detected
	...
	if mapping.DepositCol == -1 && mapping.WithdrawCol != -1 {
		mapping.DepositCol = mapping.WithdrawCol + 1
	}
...
func excelToJSON(filePath string) (*FileData, error) {
	...
	if mapping.DateCol == -1 {
		return nil, fmt.Errorf("cannot detect date column")
	}
	if mapping.WithdrawCol == -1 {
		return nil, fmt.Errorf("cannot detect amount column")
	}

	fmt.Printf("Detected mapping for %s: Date=%d, Deposit=%d, Withdraw=%d, Skip=%d\n",
		filepath.Base(filePath), mapping.DateCol, mapping.DepositCol, mapping.WithdrawCol, mapping.SkipRows)
	...
	for i, row := range rows {
		...
		withdrawAmount := parseAmount(getCell(row, mapping.WithdrawCol))
		...
		if mapping.DepositCol >= 0 {
			depositAmount := parseAmount(getCell(row, mapping.DepositCol))
			if depositAmount > 0 {
				data.Transactions = append(data.Transactions, Transaction{
					Date:   date,
	...
	runButton.OnTapped = func() {
		...
		go func() {
			...
			if err != nil {
				...
				logWriter(fmt.Sprintf("File A: %d transactions extracted -> %s",
			...
			if err != nil {
				...
				logWriter(fmt.Sprintf("File B: %d transactions extracted -> %s",
